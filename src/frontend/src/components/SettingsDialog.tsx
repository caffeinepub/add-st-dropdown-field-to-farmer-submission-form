...
import { AlertCircle, CheckCircle2, Info, Loader2, Lock } from "lucide-react";
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      const _response = await fetch(
        `${GOOGLE_APPS_SCRIPT_WEB_APP_URL}?action=test`,
        {
          method: "GET",
      ...
      if (error.name === "AbortError") {
        ...
        setErrorMessage(
          "Unable to connect to Google Sheets. Please verify your internet connection.",
        );
      }
    }
  };
      ...
      <DialogContent className="sm:max-w-[600px] bg-white">
        <DialogHeader>
          ...
          <DialogDescription className="text-base text-slate-600 mt-2">
            The Google Apps Script Web App URL is permanently configured for
            data storage.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5 py-5">
          <Alert className="bg-blue-50 border-blue-200">
            ...
            <AlertDescription className="ml-2 text-blue-800 text-sm leading-relaxed">
              <strong className="font-semibold">Important:</strong> All form
              submissions are sent directly to Google Sheets via the Google Apps
              Script URL below. Both Login ID and Farmer Name are automatically
              included in every submission for tracking purposes.
            </AlertDescription>
          </Alert>
            ...
            {connectionStatus === "success" && (
              <Alert className="bg-teal-50 border-teal-200">
                ...
                <AlertDescription className="ml-2 text-teal-800 font-medium">
                  Connection successful! The Google Apps Script is reachable and
          ...
          <div className="bg-teal-50 border border-teal-200 rounded-lg p-5 space-y-3">
            ...
            <p className="text-sm text-teal-800 leading-relaxed">
              All farmer data submissions are sent directly to Google Sheets
              with the following field order:
            </p>
            <ul className="text-sm text-teal-700 space-y-1.5 mt-3 list-disc list-inside leading-relaxed">
              <li>
                <strong className="font-semibold">Login ID:</strong>{" "}
                Automatically included from your session (1st column)
              </li>
              <li>
                <strong className="font-semibold">Farmer Name:</strong> Manually
                entered for each submission (2nd column)
              </li>
              <li>
                <strong className="font-semibold">Mobile Number:</strong>{" "}
                10-digit phone number (3rd column)
              </li>
              <li>
              ...
              <li>
                <strong className="font-semibold">Crop 1, Crop 2:</strong>{" "}
                Primary and secondary crops
              </li>
              <li>
                <strong className="font-semibold">
                  Irrigation Type and Total Acreage:
                </strong>{" "}
                Farming details
              </li>
              <li>
                <strong className="font-semibold">
                  ST, MGOHQ, WheatVariety:
                </strong>{" "}
                Additional tracking fields
              </li>
            </ul>
            <p className="text-xs text-teal-700 mt-3 leading-relaxed">
              The Google Apps Script receives these fields in the exact order
              shown above.
            </p>
          </div>
        </div>
